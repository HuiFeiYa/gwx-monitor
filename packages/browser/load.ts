...
export function setupReplace(): void {
    ...
    addReplaceHandler({
        callback: (res) => {
            ...
            const htmlString = htmlElementAsString(
                res.data.target as HTMLElement
            );
            if (htmlString) {
    ...
    addReplaceHandler({
        callback: (res) => {
            ...
            const blockElements =
                document.getElementsByClassName("mto-scroll-b"); // 获取区块元素
            const nestBlockElement =
                document.getElementsByClassName("mto-scroll-c"); // 获取第二层嵌套的滚动元素
            // 区块
            const blockElementPos = Array.from(blockElements).map(
                (item: any) => {
                    return {
            ...
            // 内部滚动元素位置展示
            const nestBlockElementPos = Array.from(nestBlockElement).map(
                (item: any) => {
                    return {
